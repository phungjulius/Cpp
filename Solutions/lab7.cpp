...
public:
	...
	RandGen(int min0 = LOTTO_MIN, int max0 = LOTTO_MAX);  
	int operator()();
private:
	int min;
	int max;
...
RandGen::RandGen(int min0, int max0)
{
	//create different sequence of random numbers on every program run by using current time as seed for randm generator
...
int main()
{
	...
	do
	{
		...
		{
			int count = 0;
			for_each(lottoVikingMatch.begin(), lottoVikingMatch.end(), [&count](int value)
				{
					count++;
					cout << "#" << count << ": " << value;
					cout << endl;
		...
		{
			int count = 0;
			for_each(allThreeMatch.begin(), allThreeMatch.end(), [&count](int value)
				{
					count++;
					cout << "#" << count << ": " << value;
					cout << endl;
